...
const diretorFilmeDAO = require("../../model/DAO/diretor_filme/diretor_filme.js")



//função para inserir
const inserirNovoDiretorFilme = async function(diretorFilme){
    ...
    try {
        ...
        let validar = await validarDados(diretorFilme)
        ...
        //se os dados estiverem corretos ele envia para o DAO 
            ...
            let result = await diretorFilmeDAO.insertDiretorFilme(diretorFilme)
            ...
            if(result){ //se o item for cadastrado corretamente ele envia uma mensagem de sucesso
                ...
                diretorFilme.id = result //pegando o id do genero cadastrado e adicionando no JSON de genero
                ...
                message.DEFAULT_MESSAGE.response = diretorFilme //aparece os dados do genero do response para o usuário conferir
...
const atualizarDiretorFilme = async function(diretorFilme, contentType, id){
    ...
    try {
        ...
        let validandoId = await buscarDiretorFilmeID(id)
        ...
        if(validandoId.status){ //vai olhar o status (true / false) da função
            ...
            let validarDadosDiretorFilme = await validarDados(diretorFilme)

            //validaddo retorno da função
            if(!validarDadosDiretorFilme){ //se a função de validação retornar "false" (dados corretos) ele cai aqui

                //adicionando o id no objeto "genero" recebido na requisição, para enviar tudo em um único objeto
                diretorFilme.id = Number(id)

                //enviando para para o DAO 
                let result = await diretorFilmeDAO.updateDiretorFilme(diretorFilme) 
                ...
                if(result){
                    ...
                    message.DEFAULT_MESSAGE.response = diretorFilme //envia os dados do filme no response, para o usuário visualizar
